...
public class ServerDataHandler implements Runnable {
    private static final Logger _logger = LogUtil
	    .getLogger(ServerDataHandler.class.getName());
    ...
    public void processData(TailServer server, SocketChannel socket, final byte[] data, int count) {
	_logger.fine("called.");
	...
	_logger.fine("called.");
	//AbstractClient abstractClient = this.abstractClientMap.putIfAbsent(dataEvent.socket, new AbstractClient(dataEvent.server, dataEvent.socket));
	AbstractClient abstractClient = this.abstractClientMap.get(dataEvent.socketChannel);
	
	if(abstractClient == null) {
	    _logger.fine("abstractClient was null");
	    //abstractClient = this.abstractClientMap.get(dataEvent.socket);
	    abstractClient = this.createAbstractClientAndPutToMap(dataEvent.tailServer, dataEvent.socketChannel);
	}
    ...
    private synchronized AbstractClient createAbstractClientAndPutToMap(TailServer tailServer, SocketChannel socketChannel) {
	_logger.fine("called.");
	if(this.abstractClientMap.get(socketChannel) == null) {
	    
	    this.abstractClientMap.put(socketChannel, new AbstractClient(tailServer, socketChannel));
