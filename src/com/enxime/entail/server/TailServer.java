...
public class TailServer implements Runnable {
    private static final Logger _logger = LogUtil.getLogger(TailServer.class.getName());
    ...
    public TailServer(InetAddress hostAddress, int port, ServerDataHandler serverDataHandler)
	    throws IOException {
	_logger.info("called");
	...
	while (true) {
		...
		synchronized (this.pendingChanges) {
		    Iterator<ServerChangeRequest> changes = this.pendingChanges.iterator();
	...
	this.serverDataHandler.processData(this, socketChannel, this.readBuffer.array(),
		numRead);
    }
	...
	synchronized (this.pendingChanges) {
	    ...
	    synchronized (this.pendingData) {
		List<ByteBuffer> queue = (List<ByteBuffer>) this.pendingData.get(abstractClient);
		if (queue == null) {
