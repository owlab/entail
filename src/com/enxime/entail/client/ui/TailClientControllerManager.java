...
public class TailClientControllerManager {
	...
	if (tailClientController != null && tailClientController.getTailUrls().size() == 0) {
		try {
		    Thread.sleep(500);
 ...
 // stop tailing a file
	...
	switch (fromServer.getResultCode()) {
		...
		CTabItemWrapperManager.getInstance().appendLinesToCTabItemBodyFromOtherThread(tailUrl, fromServer.getTailLineList());
		break;
	    case ResultCode.NOT_FOUND:
		// clientController.setMessage("file not found.");
		CTabItemWrapperManager.getInstance().setMessageToCTabItemMessageFromOtherThread(tailUrl, "file not found");
		break;
	    case ResultCode.EXCEPTION:
		CTabItemWrapperManager.getInstance().setMessageToCTabItemMessageFromOtherThread(tailUrl, fromServer.getResultMessage());
		break;
	    case ResultCode.FILE_ROTATED:
		CTabItemWrapperManager.getInstance().setMessageToCTabItemMessageFromOtherThread(tailUrl, "file rotated");
		break;
	    case ResultCode.ACK:
    ...
    public void handleTailClientException(TailClientController tailClientController, Thread thread, Throwable exception) {
	tailClientController.stopTailClientAndDataHandlerThreads();
	...
	for(TailURL tailUrl : tailUrls) {
	    ...
	    CTabItemWrapperManager.getInstance().setMessageToCTabItemMessageFromOtherThread(tailUrl, exception.getMessage());
	}
