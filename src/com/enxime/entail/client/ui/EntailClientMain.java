...
public class EntailClientMain {
    private static final Logger _logger = LogUtil.getLogger(EntailClientMain.class
	    .getName());
	...
	folder.addSelectionListener(new SelectionListener() {
		...
		_logger.fine("Selected CTabItem: " + folder.getSelection().getToolTipText());
		
		CTabItemWrapper cTabItemWrapper = CTabItemWrapperManager.getInstance().getCTabItemWrapper(folder.getSelection());
	...
	public void widgetSelected(SelectionEvent event) {
		...
		boolean success = CTabItemWrapperManager.getInstance().addCTabItemWrapper(folder,
			tailUrl);
		if(success) {
		    success = TailClientControllerManager.getInstance().addTailURL(tailUrl);
		}
		if(success) {
	...
	public void widgetSelected(SelectionEvent event) {
		...
		TailState tailState = cTabItemWrapper.getTailState();
		if(tailState == TailState.RUNNING) {
		    TailClientControllerManager.getInstance().suspendTailURL(cTabItemWrapper.getTailUrl());
		    cTabItemWrapper.setTailState(TailState.PAUSED);
		    pauseButton.setText(TailConstant.RESTART);
		    
		} else if(tailState == TailState.PAUSED) {
		    TailClientControllerManager.getInstance().restartTailURL(cTabItemWrapper.getTailUrl());
