...
public class TailClientController implements ClientProtocolListener, UncaughtExceptionHandler /*, TailClientExceptionListener*/{
	private static final Logger _logger = LogUtil.getLogger(TailClientController.class.getName());
	...
	public TailClientController(String host, int port) throws UnknownHostException, IOException {
		this.host = host;
		this.port = port;
		
		this.tailClientDataHandler = new TailClientDataHandler(this);
		this.tailClientDataHandlerThread = new Thread(this.tailClientDataHandler);
		this.tailClientDataHandlerThread.setName("TailClientDataHandler thread: " + this.tailClientDataHandlerThread.getName());
		
		this.tailClientDataHandlerThread.start();
		
		_logger.fine(this.tailClientDataHandlerThread.getName() + " started.");
		
		this.tailClient = new TailClient(InetAddress.getByName(host), port, this.tailClientDataHandler);

		this.tailClientThread = new Thread(this.tailClient);
		this.tailClientThread.setName("TailClient thread: " + this.tailClientThread.getName());
	...
	private void sendCommandToServer(String tailFilePath, String command, String parameter) throws IOException {
		_logger.fine("Command sending: " + tailFilePath + ", " + command + ", " + null);
	...
	public void handleServerResponse(ToClient fromServer) {
	    ...
	    TailClientControllerManager.getInstance().handleServerResponse(tailUrl, fromServer);
	...
	public void uncaughtException(Thread thread, Throwable exception) {
		...
		TailClientControllerManager.getInstance().handleTailClientException(this, thread, exception);
